defmodule WemachatDatabase.Schemas.Group do
  ...
  def create_changeset(group, attrs) do
    group
    |> cast(attrs, [:name, :description, :group_image_url, :creator_id, :max_members])
  ...
  @doc """
  Changeset for updating group metadata (last message, member count).
  This is typically called by trigger functions, not manually.
  """
  def metadata_changeset(group, attrs) do
    group
    |> cast(attrs, [:last_message_text, :last_message_at, :member_count])
